...
--
-- Host: 127.0.0.1
-- Loomise aeg: Märts 05, 2026 kell 04:29 PL
...
-- Toimingud
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `FilmiPikkus` (IN `pikkusMinutit` INT)  COMMENT 'linnanimi, kus arhvaarv on suure kui @arv' BEGIN
	DECLARE TODAY TIMESTAMP DEFAULT CURRENT_DATE;
	SELECT filmNimetus, pikkus, today
	FROM film
	WHERE pikkus >pikkusMinutit;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `KustutaZanr` (IN `ID` INT)   BEGIN
	SELECT * FROM zanr;
    DELETE FROM zanr
    WHERE zanrId = ID;
    SELECT * FROM zanr;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `RezisoorMuutus` (IN `ID` INT, IN `uusEesnimi` VARCHAR(30), IN `uusPerekonnaNimi` VARCHAR(30))   BEGIN
	SELECT * FROM rezisoor;
    UPDATE rezisoor
    SET eesnimi = uusEesnimi,
        perenimi = uusPerekonnaNimi
    WHERE rezisoorID = ID;
    SELECT * FROM rezisoor;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `tervist` ()  DETERMINISTIC COMMENT 'A Procedur' BEGIN
	SELECT 'Hello World!';
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ZanrLisamine` (IN `uusZanrNimetus` VARCHAR(20), IN `uusZanrKirjeldus` VARCHAR(100))   BEGIN
	SELECT * FROM zanr;
    INSERT INTO zanr (zanrNimi, zanrKirjeldus)
    VALUES (uusZanrNimetus, uusZanrKirjeldus);
    SELECT * FROM zanr;
END$$
...
(11, 'Ulme', 'Filmid kosmosereisidest, paralleelmaailmadest ja tuleviku uskumatutest tehnoloogiatest.');
...
--
ALTER TABLE `zanr`
  MODIFY `zanrID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=12;
